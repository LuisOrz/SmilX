...
class initial_parameters:
    def __init__(self):
        ...
        if logo_uri:
            st.markdown(
                ...
                <div style="display:flex;justify-content:center;align-items:center;
                            background:#000000;border-radius:14px;padding:2.5rem 3rem;
                            margin:0 auto 2rem auto;max-width:640px;
                            border:1px solid rgba(255,255,255,0.07);">
                    <img src="{logo_uri}" style="width:100%;max-width:400px;height:auto;display:block;" />
                </div>
                """,
        ...
        st.markdown(
            ...
            <div style="text-align:center;font-family:'DM Sans',sans-serif;font-size:13.5px;
                        line-height:1.8;color:rgba(240,242,241,0.65);
    ...
    def ask_molecular_formula(self):
        try:
            self.str_molecular_formula = st.text_input(
                "Molecular formula", value="C6H14", key="first",
    ...
    def get_molecular_formula(self):
        ...
        if elements_y_atoms:
            ...
            for i_tuple in elements_y_atoms:
                self.molecular_formula[i_tuple[0]] = 1 if i_tuple[1] == '' else int(i_tuple[1])
    ...
    def reorder_molecular_formula(self):
        ...
        for i_element in 'C','H','B','Br','Cl','F','I','N','O','P','S':
            if self.molecular_formula[i_element] > 0:
                n = self.molecular_formula[i_element]
                self.str_molecular_formula += i_element if n == 1 else f'{i_element}{n}'
    ...
    def get_syntax_rules(self):
        ...
        if n in {1,2,3}:
            self.syntax_rules = [[0]] * n
        elif n in {4,5,6}:
            self.syntax_rules = [[0],[0]] + [[0,1]]*(n-3) + [[0]]
        elif n == 7:
